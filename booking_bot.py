def book_parking(driver):
    try:
        ...
        # Step 2: Wait for and click second Reserve button with modal handling
        max_attempts = 3
        for attempt in range(max_attempts):
            try:
                ...
                # Check for and close any modal overlay
                try:
                    modal_overlay = driver.find_element(By.XPATH, "//div[contains(@class, 'modal black-overlay')]")
                    close_button = modal_overlay.find_element(By.XPATH, ".//button[contains(@class, 'close')]")
                    close_button.click()
                    WebDriverWait(driver, 5).until_not(
                        EC.presence_of_element_located((By.XPATH, "//div[contains(@class, 'modal black-overlay')]"))
                    )
                    print(f"[INFO] Closed modal overlay on attempt {attempt + 1}")
                except NoSuchElementException:
                    print(f"[INFO] No modal overlay found on attempt {attempt + 1}")
